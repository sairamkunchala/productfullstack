...
const Owner = () =>{
    ...
    const handleSubmit =async event => {
        ...
        if(logData.email==="" || logData.mobilenumber==="" || logData.password===""){
            ...
            try{
            const response =  await axios.post("http://localhost:5000/api/login",logData).then((res) => {if(res.data === "success"){ navigate("/productpage")} else{console.log("falserequesting")} }).catch((err) => toast.error(err.response.data));
